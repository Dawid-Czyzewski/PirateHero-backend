...
final class Version20260706120000_PalacBestiaryAndTitle extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'Add poseidon_champion title and backfill Palac bestiary entries for existing dungeon progress';
    }
    ...
    public function up(Schema $schema): void
    {
        $this->addSql("INSERT INTO player_title (code, name_key, description_key, unlock_type, unlock_value, unlock_dungeon_id, sort_order) VALUES
            ('poseidon_champion', 'titles.poseidon_champion.name', 'titles.poseidon_champion.unlockHint', 'DUNGEON_COMPLETED', NULL, 'palac', 8)");
        ...
        $this->addSql("INSERT INTO user_title (user_id, player_title_id, unlocked_at)
            ...
            INNER JOIN player_title pt ON pt.code = 'poseidon_champion' AND pt.unlock_dungeon_id = 'palac'
            WHERE udp.dungeon_id = 'palac' AND udp.cleared_stage >= 10
            AND NOT EXISTS (
    ...
    {
        $this->addSql("DELETE FROM user_title WHERE player_title_id IN (SELECT id FROM player_title WHERE code = 'poseidon_champion')");
        $this->addSql("DELETE FROM user_bestiary_entry WHERE dungeon_id = 'palac'");
        $this->addSql("DELETE FROM player_title WHERE code = 'poseidon_champion'");
    }
}
