...
final class Version20260702113000_WulkanBestiaryAndTitle extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'Add volcanic_conqueror title and backfill Wulkan bestiary entries for existing dungeon progress';
    }
    ...
    public function up(Schema $schema): void
    {
        $this->addSql("INSERT INTO player_title (code, name_key, description_key, unlock_type, unlock_value, unlock_dungeon_id, sort_order) VALUES
            ('volcanic_conqueror', 'titles.volcanic_conqueror.name', 'titles.volcanic_conqueror.unlockHint', 'DUNGEON_COMPLETED', NULL, 'wulkan', 7)");
        ...
        $this->addSql("INSERT INTO user_title (user_id, player_title_id, unlocked_at)
            ...
            INNER JOIN player_title pt ON pt.code = 'volcanic_conqueror' AND pt.unlock_dungeon_id = 'wulkan'
            WHERE udp.dungeon_id = 'wulkan' AND udp.cleared_stage >= 10
            AND NOT EXISTS (
    ...
    {
        $this->addSql("DELETE FROM user_title WHERE player_title_id IN (SELECT id FROM player_title WHERE code = 'volcanic_conqueror')");
        $this->addSql("DELETE FROM user_bestiary_entry WHERE dungeon_id = 'wulkan'");
        $this->addSql("DELETE FROM player_title WHERE code = 'volcanic_conqueror'");
    }
}
